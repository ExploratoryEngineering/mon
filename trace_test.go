...
func TestTracing(t *testing.T) {
	...
	resp, err := http.Post(fmt.Sprintf("%s/mon/trace", ep.ServerURL()), "text/plain", strings.NewReader("1"))
	if err != nil {
	...
	resp, err = http.Post(fmt.Sprintf("%s/mon/trace", ep.ServerURL()), "text/plain", strings.NewReader("a long long time"))
	if err != nil {
		t.Fatalf("Couldn't do request to trace: %v", err)
	}
	...
	list, _ := f.Readdir(99)
	for _, v := range list {
		if strings.HasPrefix(v.Name(), "trace_20") {
			t.Logf("Removing %s\n", v.Name())
			os.Remove(v.Name())
		}
	}
